...
public class BlockFileOperator {
  ...
  private final BlockLock mBlockLock;
  private final String mFilePath;
  private final RandomAccessFile mLocalFile;
  private final FileChannel mLocalFileChannel;
  private final Closer mCloser = Closer.create();

  public BlockFileOperator(BlockMeta blockMeta, BlockLock blockLock) throws FileNotFoundException {
    // Check if the block to operate is guarded by the correct lock
    Preconditions.checkState(blockMeta.getBlockId() == blockLock.getBlockId());
    mBlockMeta = Preconditions.checkNotNull(blockMeta);
    mBlockLock = Preconditions.checkNotNull(blockLock);
  ...
  public ByteBuffer read(long offset, long length) throws IOException {
    Preconditions.checkState(mBlockLock.isLocked());
